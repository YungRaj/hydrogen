...
Solids inventory defaults (B1): `d_p = 0.13 mm`, metal loading `0.5`, dispersion `0.3`. Area law: `a = a_geom × loading × dispersion` (both ≤ 1), applied per bed volume (`A = a · V_bed`; PFR) and per emulsion volume (fluidized, `ε_mf = 0.45`). The pre-merge per-gas-volume basis undercounted area by `1/ε ≈ 2.5×`. On the production cell, isothermal Ar-tracer X at 1300 K is PFR **2.90%**, fluidized **1.19%** (emulsion 2.38%, bubble bypass δ = 0.5) for `cat_9`; envelope 1×1 is 19.32% / 7.91%. The PFR figure is the site-inventory bound `Γ·a / (ε·P/RT)` = 0.104 / 3.56 mol m⁻³ to three figures: monolayer inventory, not kinetics. `cat_9` is a SAC, so its MMBCR cells are `not_applicable`. Flotation default is unconstrained (`η = 1`). Open work: [`docs/backlog/`](docs/backlog/) — B6-5 before B5.
