...
from pipeline.data_models.stages import (
    CandidateBatch, DFTProducts, DFTState, DiscoveryProducts, DiscoveryState,
    FuelCellProducts, FuelCellState, ReactorBatchProducts, ReactorBatchState,
    ReportProducts, ReportState, SelectedCandidates, TableLike,
    VQEProducts, VQEState,
)
...
class ReactorBatchStage(Protocol):
    ...
    def __call__(
            self, candidates: TableLike | None, *, temperatures: Sequence[float],
...
class DFTStage(Protocol):
    ...
    def __call__(self, candidates: CandidateBatch, *, top_k: int,
                 execute_dft: bool,
...
class ReportStage(Protocol):
    ...
    def __call__(self, pipeline_state: PipelineStateDocument
...
class PipelineRuntime:
    ...
    save_state: Callable[[PipelineState], None]
...
def default_pipeline_runtime() -> PipelineRuntime:
    ...
    """
    from pipeline.utils import load_json  # pyright: ignore[reportUnknownVariableType]
    from pipeline.utils import print_banner
    from pipeline.utils import save_json  # pyright: ignore[reportUnknownVariableType]

    typed_load_json = cast(Callable[[str], object], load_json)
    typed_save_json = cast(
        Callable[[dict[str, Any], str], Path], save_json)

    def load() -> PipelineState:
        return validate_pipeline_state(
            typed_load_json('pipeline_state.json') or {})

    def save(value: PipelineState) -> None:
        typed_save_json(dict(value), 'pipeline_state.json')
...
def default_pipeline_components() -> PipelineComponents:
    ...
    from pipeline.stages.dft import run_dft_stage  # pyright: ignore[reportUnknownVariableType]
    from pipeline.stages.discovery import run_discovery_stage
    from pipeline.stages.fuel_cell import run_fuel_cell_stage
    from pipeline.stages.reactor_batch import run_reactor_batch_stage  # pyright: ignore[reportUnknownVariableType]
    from pipeline.stages.report import run_report_stage  # pyright: ignore[reportUnknownVariableType]
    from pipeline.stages.vqe import run_vqe_stage  # pyright: ignore[reportUnknownVariableType]
    return PipelineComponents(
        discovery=cast(DiscoveryStage, run_discovery_stage),
        reactor_batch=cast(ReactorBatchStage, run_reactor_batch_stage),
        dft=cast(DFTStage, run_dft_stage),
        vqe=cast(VQEStage, run_vqe_stage),
        fuel_cell=cast(FuelCellStage, run_fuel_cell_stage),
        report=cast(ReportStage, run_report_stage),
        load_candidates=cast(CandidateLoader, load_selected_candidates))
...
class MemoryStateStore:
    ...
    def __init__(self, initial: Mapping[str, Any] | None = None):
        self.state: PipelineState = validate_pipeline_state(
            deepcopy(dict(initial or {})))
    ...
    def save(self, value: PipelineState) -> None:
        """Atomically save the surrogate model.

        Args:
            value: Pipeline state value to copy or persist.
        """
        validated = validate_pipeline_state(value)
        self.state = deepcopy(validated)
        self.history.append(deepcopy(validated))
